...
let arianeeTransferFunction = arianeeContract.GetFunction("approve");
//multisig contract addrass as data for approve function 
let data = arianeeTransferFunction.CreateCallInput(multiSigContractAddress).Data;
let byteDataApprove = data.HexToByteArray();
//call transaction 
...
let transactionHash = await multiSigExecuteFunction.SendTransactionAsync(account.Address, gasApprove, null, arianeeContractAddress, 0, byteDataApprove);
...
let data = arianeeTransferFunction.CreateCallInput(newAddress).Data;
let byteDataTransfer = data.HexToByteArray();
//call transaction 
let gasTransfer = await multiSigExecuteFunction.EstimateGasAsync(arianeeContractAddress, 0, byteDataTransfer);
let transactionHash = await multiSigExecuteFunction.SendTransactionAsync(account.Address, gasTransfer, null, arianeeContractAddress, 0, byteDataTransfer);
...
let transactionHash = await multiSigExecuteFunction.SendTransactionAsync(account.Address, gas, null, arianeeContractAddress, 0, byteDataTransfer);